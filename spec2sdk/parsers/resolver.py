...
class ResolvingParser:
    ...
    def _resolve_schema(self, relative_filepath: Path, schema: dict) -> dict:
        def resolve_value(value: Any) -> Any:
            if isinstance(value, dict):
                return self._resolve_schema(relative_filepath, value)
        ...
        def resolve_reference(reference: str) -> dict:
            if reference not in self._resolved_references:
                ...
                if parsed_ref.scheme == parsed_ref.netloc == "":
                    ...
                    } | self._resolve_schema(relative_filepath=ref_relative_filepath, schema=ref_schema_fragment)
                else:
